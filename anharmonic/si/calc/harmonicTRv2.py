    ...
    for i in range(split): #average across split number of chunks
        ...
        for j in range(tn):
            ...
            if j == 0: #avoid divide by zero
                qom[j,0] = 0+0j
#            else:
#                qom[j,0] = -(lvelsfft[j,:]*(kij*rvelsfft[j,:].conj()).sum(axis=1)).sum()/om[j]
            else:
                qom[j,0] = -(np.dot(lvelsfft[j,:], 
                   np.dot(kij,rvelsfft[j,:].transpose().conj()))/om[j])
#            else:
#                qom[j,0] = -(np.linalg.multi_dot([lvelsfft[j,:],kij,rvelsfft[j,:].transpose().conj()]))/om[j]
