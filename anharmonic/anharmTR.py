...
with open(velsfile,'r') as fid:
    ...
    for s in range(1):#split): #loop over blocks for averaging
        ...
        vfft2 = np.append(vfft[tn/2:,:],vfft[0:tn/2,:],axis=0)
        #rearange to (-freqmax,freqmax]
        ...
        for i in range(1):#nl) #loop over left atoms    
            ...
            vi = vfft[:,idsl[i]*3:idsl[i]*3+3] #v x,y,z on left side
            for j in range(1):#nr): #loop over right atoms
                vj = np.flipud(vfft[:,idsr[j]*3:idsr[j]*3+3]) #v x,y,z on left 
...
#                    for w in range(2):#tn/2): #positive frequencies
#                        viw = vi[tn/2+w,:]
#                        vjwp = np.roll(vj,w,axis=0)
