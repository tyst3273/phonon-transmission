clear
CS='prist'; % the prefix of file names 
dT=80; % temperature difference used in NEMD simulations
...
kn=max(kvals);
tn=100000; % The number of velocity frames used for each piece: tn*kn should <= the total number of velocity frames collected
Nt=tn;

file_forces=strcat(CS,'.Fij.dat'); % the force constant file
file_compact_vels=strcat(CS,'.vels.compact.dat'); % the compactified velocity file

dt_md=0.8e-15; % MD time step, should be changed according to your MD simulation settings
...
dom=oms_fft(2)-oms_fft(1);
dwin=0.3e12*(2*pi); % THz change this value to adjust the convolution width
