...
dT=30 % temperature difference used in NEMD simulations

% Chunk index
kvals=1:5; % devide the whole velocity trajectory into 20 pieces, that is to get 20 independent transmission functions for averages
kn=max(kvals);
tn=2000; % The number of velocity frames used for each piece: tn*kn should <= the total number of velocity frames collected
Nt=tn;
